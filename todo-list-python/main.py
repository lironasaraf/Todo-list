...
class TaskListApp:
    ...
    def create_listbox(self):
        self.listbox = Listbox(self.root, width=55, height=14, font=('Arial', 13), bg=self.listbox_color,
                               selectbackground=self.select_bg_color, selectforeground='orange', bd=0)
        self.listbox.place(x=10, y=60)

    def create_entry(self):
        self.entry = Entry(self.root, width=55, font=('Arial', 12), bd=1, bg=self.entry_color)
    ...
    def create_buttons(self):
        ...
        for idx, (text, command) in enumerate(buttons):
            Button(self.root, text=text, command=command, bg=self.button_color, fg='white', bd=0,
                   font=('Arial', 12, 'bold'), width=6).place(x=525, y=60 + idx * 35)
...
if __name__ == '__main__':
    root = Tk()
    app = TaskListApp(root, 600, 410, 390, 160, 'ToDo List')
    app.draw_widgets()
    root.mainloop()
